...
{
    final public const string NAME = 'kronika.time';
    ...
    public function convertToDatabaseValue(mixed $value, AbstractPlatform $platform): ?string
    {
        ...
        if (! $value instanceof Time) {
            throw InvalidType::new($value, $this->getName(), ['null', Time::class]);
        }
    ...
    public function convertToPHPValue(mixed $value, AbstractPlatform $platform): ?Time
    {
        ...
        try {
            ...
            return Time::tryOfFormat($platform->getTimeFormatString(), $value)
                ?? Time::ofDateTime(new \DateTimeImmutable($value));
        } catch (\Throwable $e) {
